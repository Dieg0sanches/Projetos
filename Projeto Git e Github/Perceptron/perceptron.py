...
#ve se classificamos o ponto errado ou certo
...
def Perceptron(T, theta, lr, data, size, wall):
    #pegando a matrix de dados pro caso 3D
    if(size == 100):
        data = data.to_numpy().T
        #adicionando a parte do bias conforme número de amostras
        aux = np.full(data.shape[1], 1)
    ...
    for t in range(T):
        ...
        if (flag == 0):
            print (f'fim do treino do ciclo {t}')
            break
